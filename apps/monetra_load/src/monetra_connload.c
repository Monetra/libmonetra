...
static M_timeval_t        track_lastttime;
...
static M_bool cli_string_cb(char short_opt, const char *long_opt, const char *string, void *thunk)
{
    ...
    switch (short_opt) {
        case 'U':
            M_str_copy(config_username, sizeof(config_username), string);
            break;
        case 'P':
            M_str_copy(config_password, sizeof(config_password), string);
            break;
        case 'H':
            M_str_copy(config_hostname, sizeof(config_hostname), string);
            break;
        default:
            return M_FALSE;
    }
...
static void print_config(void)
{
    ...
    M_printf(
        ...
        "  Trans Delay ms   : %zu\r\n"
        "\r\n",
        config_hostname, (int)config_port, config_num_conns, config_trans_per_conn, config_delay_per_trans_ms);
}
...
static void create_connection(M_event_t *event)
{
    ...
    if (conn == NULL) {
        M_printf("Failed to create new LM_conn_t object\r\n");
        exit(1);
    }
    ...
    LM_conn_set_tls_clientctx(conn, tlsctx);
...
int main(int argc, const char *const *argv)
{
    ...
    timer = M_event_timer_create(event, status_callback, NULL);
